#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Ermittelt, welche Branches gegenueber "main" veraltet sind - also einen
Rebase brauchen, bevor sie weiter bearbeitet werden (f-reiser/reiser-flow#7).

Reine Vorpruefung ohne Claude: laeuft im Schritt "vorpruefung" von
claude-aufgaben.yml, der branches.txt bereits fuer die Branch-Zuordnung
erzeugt (siehe zuordnung.py). Das Ergebnis geht in einen Step-Output, den
der anschliessende Claude-Aufruf ausliest, um veraltete Branches
aufzufrischen - GitHub-Pruefungen kosten nichts, ein Claude-Aufruf schon.
"""
import io
import os
import subprocess
...
#  Der Workflow, in den dieses Skript eingehaengt gehoert - die Verdrahtung ist
#  Teil der Zusage und wird im Selbsttest mitgeprueft (siehe
#  label_sicherheitsnetz.py, wo derselbe Fall schon einmal auffiel).
WURZEL = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
WORKFLOW = os.path.join(WURZEL, "workflows", "claude-aufgaben.yml")


def wird_aufgerufen(text):
    """Ruft der Workflow-Text dieses Skript auf - Kommentarzeilen zaehlen nicht,
    die erklaeren den Aufruf nur."""
    t = chr(10).join(z for z in text.split(chr(10))
                     if not z.lstrip().startswith("#"))
    return "veraltete_branches.py" in t
...
def selbsttest():
    ...
    #  Ein Baustein, der nicht aufgerufen wird, ist kein Baustein.
    try:
        yml = io.open(WORKFLOW, encoding="utf-8").read()
    except OSError as e:
        fehler.append("claude-aufgaben.yml nicht lesbar: %s" % e)
    else:
        if not wird_aufgerufen(yml):
            fehler.append("claude-aufgaben.yml ruft veraltete_branches.py nicht "
                          "auf - die Vorpruefung haengt nicht")
    if wird_aufgerufen("#  ruft veraltete_branches.py auf"):
        fehler.append("wird_aufgerufen(): Kommentarzeile zaehlt mit")
